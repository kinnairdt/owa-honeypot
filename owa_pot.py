...
def auth():
    ...
    if request.method == 'GET':
        ...
        logger.info(f"{request.base_url} | {ip} | {ua} | {request.full_path} | GET | {username}:{password}")
        return redirect('/owa/auth/logon.aspx?replaceCurrent=1&reason=2&url=', 302)

@app.route('/autodiscover/<vuln_path>', methods=['GET', 'POST'])
@changeheader
def autodiscover(vuln_path):
    ua = request.headers.get('User-Agent')
    ip = request.remote_addr
    if request.method == 'GET':
        logger.info(f"{request.base_url} | {ip} | {ua} | {request.full_path} | GET")
        return redirect('/owa/auth/logon.aspx?replaceCurrent=1&reason=2&url=', 302)
    else:
        request_data = request.get_json()
        logger.info(f"{request.base_url} | {ip} | {ua} | {request.full_path}| POST | {request_data}")
        return Response('', status=301, mimetype='application/json')
...
def owa():
    ua = request.headers.get('User-Agent')
    ip = request.remote_addr
    logger.info(f"{request.base_url} | {ip} | {ua}")
